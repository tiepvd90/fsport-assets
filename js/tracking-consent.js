(function () {
  ...
  function renderWhenEnabled() {
    loadBannerSetting(function (config) {
      ...
      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", function () { render(config.message); }, { once: true });
      } else render(config.message);
    });
  }
  ...
  function render(message) {
    ...
    style.textContent =
      "#fsTrackingConsent{position:fixed;z-index:30000;bottom:calc(72px + env(safe-area-inset-bottom));left:50%;width:calc(100% - 32px);max-width:900px;box-sizing:border-box;display:flex;align-items:center;justify-content:space-between;gap:20px;margin:0;padding:18px 20px;border:1px solid #d8dde6;border-radius:14px;background:#fff;color:#172033;box-shadow:0 16px 45px rgba(15,23,42,.24);font:14px/1.55 \"Be Vietnam Pro\",system-ui,sans-serif;transform:translateX(-50%)}" +
